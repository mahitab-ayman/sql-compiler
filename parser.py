...
Implements recursive descent parsing and generates parse trees
"""
...
class SyntaxAnalyzer:
    ...
    def consume(self, expected_type, expected_value=None):
        ...
        if token is None:
            self.report_error(f"Unexpected end of file. Expected {expected_type}" + 
        ...
        if token.token_type == expected_type:
            if expected_value is None or token.lexeme == expected_value:
                ...
                self.report_error(f"Expected '{expected_value}' but found '{token.lexeme}'", 
                                token.line, token.column)
                return None
        else:
            self.report_error(f"Expected {expected_type}" + 
                            (f" '{expected_value}'" if expected_value else "") + 
                            f" but found {token.token_type} '{token.lexeme}'",
                            token.line, token.column)
            return None
    ...
    def synchronize(self):
        ...
        while self.current_index < len(self.tokens):
            ...
            # Synchronize on major keywords or semicolon
    ...
    def parse_expression(self):
        ...
        if token.token_type == 'IDENTIFIER':
            ...
            self.advance()
            return ParseTreeNode('LITERAL', token.lexeme, 
                              line=token.line, column=token.column,
                              data_type=token.token_type.replace('_LITERAL', ''))
        else:
            return None
