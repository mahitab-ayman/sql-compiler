...
class ParseTreeVisualizer:
    ...
    def _print_ascii_node(self, node, prefix, is_last, is_root=False):
        """Recursively print ASCII tree using simple ASCII characters"""
        if node is None:
            return
        
        # Use simple ASCII characters that work on all encodings (Windows cp1256, etc.)
        if is_root:
            connector = ""
            new_prefix = prefix
        else:
            # Use simple ASCII characters: +-- for branches, | for vertical lines
